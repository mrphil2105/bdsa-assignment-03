...
public class TaskRepositoryTests
{
    ...
    public void Create_CreatesTaskEntity_WhenGivenTaskDTO()
    {
        ...
        var r2 = taskRepo.Create(new TaskCreateDTO(
    ...
    public void Update_UpdatesStateOfTask()
    {
        ...
        var taskRepo = new TaskRepository(_context);
        var (_, t1) = taskRepo.Create(new TaskCreateDTO("cool title", null, "man, i cant wait to do this task", tags));
        var r = taskRepo.Update(new TaskUpdateDTO(t1, "updated title", null, "man, i cant wait to do this task", tags,
            State.Active));
        Assert.Equal(Response.Updated, r);

        var task = taskRepo.ReadAll()
            .FirstOrDefault();
    ...
    public void ReadAllRemoved_ReturnsTaskDTO()
    {
        ...
        var taskRepo = new TaskRepository(_context);
        var (_, t1) = taskRepo.Create(new TaskCreateDTO("cool title", null, "man, i cant wait to do this task", tags));
        taskRepo.Update(new TaskUpdateDTO(t1, "updated title", null, "man, i cant wait to do this task", tags,
            State.Active));
        taskRepo.Delete(t1);
        var r = taskRepo.ReadAll()
            .FirstOrDefault();
